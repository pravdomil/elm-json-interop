...
type_ aDecoder bDecoder =
    BD.map15 (\a b c d e f g h i j k l m n o -> { bool = a, int = b, float = c, char = d, string = e, unit = f, tuple2 = g, tuple3 = h, list = i, array = j, record = k, maybe = l, result = m, set = n, dict = o }) (D.field "bool" D.bool) (D.field "int" D.int) (D.field "float" D.float) (D.field "char" BD.char) (D.field "string" D.string) (D.field "unit" (D.succeed ())) (D.field "tuple2" (D.map2 Tuple.pair (D.field "a" aDecoder) (D.field "b" bDecoder))) (D.field "tuple3" (D.map3 (\a b c -> ( a, b, c )) (D.field "a" aDecoder) (D.field "b" bDecoder) (D.field "c" bDecoder))) (D.field "list" (D.list (D.map2 (\a b -> { a = a, b = b }) (D.field "a" aDecoder) (D.field "b" bDecoder)))) (D.field "array" (D.array (D.map2 (\a b -> { a = a, b = b }) (D.field "a" aDecoder) (D.field "b" bDecoder)))) (D.field "record" (D.succeed {})) (BD.maybeField "maybe" (D.nullable aDecoder)) (D.field "result" (BD.result D.int aDecoder)) (D.field "set" (BD.set D.int)) (D.field "dict" (BD.dict D.int aDecoder))
